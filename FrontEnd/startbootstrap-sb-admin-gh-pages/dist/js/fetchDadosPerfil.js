function refreshDadosPerfil() {
    async function fetchAsync() {
      ...
      const response = await fetch(`http://localhost:3000/operational/rest/${id_operacional}`);
      const users = await response.json();
      const response1 = await fetch(`http://localhost:3000/operational/getOperationalTeam/${id_operacional}`);
